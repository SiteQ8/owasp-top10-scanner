# Fix the regex warning and create requirements file
import re
...
# Fix the regex escape sequence warning
content = content.replace(r"(\d+)/tcp\s+open\s+(\S+)", r"(\\d+)/tcp\\s+open\\s+(\\S+)")
...
requirements = '''# OWASP Top 10 Security Scanner Requirements
requests>=2.32.0
colorama>=0.4.6
urllib3>=2.0.0
'''
...
readme_content = '''# OWASP Top 10 Security Scanner with Nmap Integration
...
print("📋 Created files:")
print("  - owasp_scanner.py (main scanner tool)")  
